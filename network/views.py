...
from transform import simulate_cat_vision_video, simulate_dog_vision_video
...
def index(request):
    ...
    for post in post_list:
        ...
        new_type.append({"post": post, "is_liked": is_liked, "video_path": video_path})
...
def create_post(request):
    ...
    if request.method == "POST":
        ...
        length = Post.objects.all().count() + 1
        post = Post(poster=poster,content=content,video_file=video_file, length=length)
        ...
        print(output_path)
        transformed_video = simulate_dog_vision_video(post.video_file.path, output_path)
        print(post.time)
        return HttpResponseRedirect(reverse("index"))
    else :
        return HttpResponseRedirect(reverse("index"))
